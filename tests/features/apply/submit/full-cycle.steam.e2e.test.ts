...
function startNextDevServer(opts: { port: number; env: Record<string, string> }) {
  ...
  const child = spawn(process.execPath, [nextBin, 'dev', '-p', String(opts.port), '-H', '127.0.0.1'], {
    cwd: process.cwd(),
    env: {
      ...process.env,
      ...opts.env,
...
describe('Apply workflow: submit endpoint (e2e over HTTP)', () => {
  beforeAll(async () => {
    ...
    // Ensure any in-process DB access uses the same database file as the server.
  ...
  it(
    ...
    async () => {
      ...
      // Seed a Steam session + identity directly into the same DB file the server uses.
      const { dbOperations } = await import('../../../../src/platform/db');
      const sessionId = crypto.randomUUID();
      dbOperations.createSteamSession({ id: sessionId, redirect_path: '/en/apply' });
      dbOperations.setSteamSessionIdentity(sessionId, { steamid64, persona_name: 'Test Persona' });

      // Ensure clean slate for this user (uniqueness constraints are part of the full flow).
