...
async function postSubmitWithSteamSession(opts: {
  ...
  dbOperations.setSteamSessionIdentity(sid, { steamid64: opts.steamid64, persona_name: 'Test Persona' });
...
describe('Apply workflow: submit endpoint (live Steam via API route)', () => {
  beforeAll(async () => {
    // Use an isolated DB for tests.
    const os = await import('node:os');
    const path = await import('node:path');
    const ts = new Date().toISOString().replace(/[:.]/g, '-');
    process.env.DB_PATH = path.join(os.tmpdir(), `triad-tactics-test-${ts}-${crypto.randomUUID()}.db`);

    // Clean any residue in case of reuse.
    const { dbOperations } = await import('@/platform/db');
    dbOperations.clearAll();
  });

  it('accepts a public profile that owns Arma Reforger', async () => {
    process.env.STEAM_WEB_API_KEY = requireEnv('STEAM_WEB_API_KEY');
    const steamid64 = requireEnv('TEST_STEAMID64_OWNED');

    const res = await postSubmitWithSteamSession({ steamid64, body: buildApplicationPayload('en') });
    expect(res.status).toBe(201);
    const json = await res.json();
    expect(json.success).toBe(true);
  });

  it('rejects a public profile that does not own Arma Reforger', async () => {
    process.env.STEAM_WEB_API_KEY = requireEnv('STEAM_WEB_API_KEY');
    ...
    const json = await res.json();
    expect(json.error).toBe('steam_game_required');
  });

  it('rejects a private profile', async () => {
    process.env.STEAM_WEB_API_KEY = requireEnv('STEAM_WEB_API_KEY');
    const steamid64 = requireEnv('TEST_STEAMID64_PRIVATE');

    const res = await postSubmitWithSteamSession({ steamid64, body: buildApplicationPayload('en') });
    expect(res.status).toBe(400);
    const json = await res.json();
    expect(json.error).toBe('steam_profile_private');
  });
  ...
  it('enforces uniqueness per steamid64 (second submit is duplicate)', async () => {
    ...
    const res1 = await postSubmitWithSteamSession({ steamid64, body: buildApplicationPayload('en'), ip: '203.0.113.11' });
    expect(res1.status).toBe(201);

    const res2 = await postSubmitWithSteamSession({ steamid64, body: buildApplicationPayload('en'), ip: '203.0.113.12' });
  ...
  it('normalizes unsupported locale to en', async () => {
    ...
    const res = await postSubmitWithSteamSession({ steamid64, body: buildApplicationPayload('de') });
    expect(res.status).toBe(201);
