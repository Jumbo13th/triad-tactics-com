...
beforeAll(() => {
  // Ensure tests fail fast if something accidentally tries to call Steam.
  delete process.env.STEAM_WEB_API_KEY;
});

describe('Apply workflow: submit endpoint (integration: validation + auth gating)', () => {
  ...
  it('accepts missing city/country (validation passes) and still gates on Steam session', async () => {
    ...
    const body: Record<string, unknown> = buildApplicationPayload({ city: undefined, country: undefined });
    delete body.city;
    delete body.country;
    ...
    expect(res.status).toBe(400);
    const json = await res.json();
    // If city/country were still required, we'd get validation_error.
    expect(json.error).toBe('steam_not_connected');
  });
});
