      ...
      {props.steamConnected ? (
        ...
        <div id="steam-auth" className="rounded-2xl border border-neutral-800 bg-neutral-950 p-4">
          <div className="flex flex-col gap-4">
            <div>
              <p className="text-sm font-medium text-neutral-200">{tf('steamAuth.title')}</p>
              <p className="mt-2 text-xs text-neutral-400">{tf('steamAuth.help')}</p>

              <div className="mt-3 rounded-xl border border-neutral-800 bg-neutral-950/40 p-3">
                <p className="text-xs font-medium text-neutral-200">{tf('steamAuth.detect.title')}</p>
                ...
                <p className="mt-2 text-xs text-neutral-400">{tf('steamAuth.detect.incognitoCheck')}</p>
              </div>
            </div>

            <div className="flex flex-wrap items-center gap-3">
              <span className="text-sm text-neutral-300">{tf('steamAuth.clickToConnect')}</span>
              <SteamSignInButton
